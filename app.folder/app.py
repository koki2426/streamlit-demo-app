...
#            散布図：デフォルト=時系列（サンプル番号×値）/ 通常 / 累積件数 / 値ごとの件数 / 小計（ランニングサム）
#            軸反転＆PDF出力対応【完全版】
...
def register_japanese_fonts():
    """ReportLab/Matplotlib に日本語フォントを登録。
       1) ./fonts/NotoSansJP-*.ttf があれば埋め込み
       2) 無ければ CID フォント HeiseiKakuGo-W5 にフォールバック（PDFのみ日本語可）
    """
    ...
    if noto_reg.exists():
        try:
            ...
            st.error(f"NotoSansJP の登録に失敗: {e}")

    if RL_FONT_REG is None:
        try:
            pdfmetrics.registerFont(UnicodeCIDFont('HeiseiKakuGo-W5'))
            RL_FONT_REG = RL_FONT_BOLD = 'HeiseiKakuGo-W5'
            st.sidebar.warning("NotoSansJP が無いため CID フォント(HeiseiKakuGo-W5)でPDFを出力します。")
        except Exception as e:
...
def clean_data(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    for c in dfc.columns:
        if dfc[c].dtype == "object":
            m = special_mask(dfc[c]); masks[c] = m
...
def basic_stats(df: pd.DataFrame, masks: dict, baseline: dict | None = None) -> pd.DataFrame | None:
    ...
    for c in cols:
        ...
        try:
            if base != "" and pd.notna(base): base = round(float(base), 2)
        except: pass
        cnt = int(data.count())
        rows.append({
            "列名": c,
...
def calc_cpk(df: pd.DataFrame, usl: float, lsl: float) -> pd.DataFrame:
    ...
    for c in numeric_cols(df):
        s = df[c].dropna()
        if len(s) <= 1:
            out.append({"列名": c, "平均値": "N/A", "標準偏差": "N/A", "Cp": "計算不可", "Cpk": "計算不可"}); continue
        mu, sd = float(s.mean()), float(s.std(ddof=1))
        if sd == 0:
            out.append({"列名": c, "平均値": round(mu, 2), "標準偏差": 0, "Cp": "計算不可", "Cpk": "計算不可"}); continue
        cp = (usl - lsl) / (6 * sd)
...
def hist_with_gauss(df: pd.DataFrame, col: str, usl: float | None, lsl: float | None):
    ...
    fig.update_layout(title=f"{col} のヒストグラム", xaxis_title=col, yaxis_title="頻度",
                      hovermode="x unified", legend=dict(x=0.7, y=1))
    if usl is not None:
...
# ========================= PDF用 図作成ヘルパー =========================
def make_series_scatter_fig(df: pd.DataFrame, y_col: str,
                            fontname: str | None = None, flip: bool = False):
    """サンプル番号(1..N) × 値 の散布図（PDF用）"""
    df2 = df[[y_col]].dropna().copy()
    df2["サンプル番号"] = np.arange(1, len(df2) + 1)
    fig, ax = plt.subplots(figsize=(6, 3))
    if flip: ax.scatter(df2[y_col], df2["サンプル番号"], s=10)
    else:    ax.scatter(df2["サンプル番号"], df2[y_col], s=10)
    title = f"{y_col}（サンプル番号×値）"
    if fontname:
        ax.set_title(title, fontname=fontname)
        if flip:
            ax.set_xlabel(y_col, fontname=fontname); ax.set_ylabel("サンプル番号", fontname=fontname)
        else:
            ax.set_xlabel("サンプル番号", fontname=fontname); ax.set_ylabel(y_col, fontname=fontname)
    else:
        ax.set_title(title)
        if flip:
            ax.set_xlabel(y_col); ax.set_ylabel("サンプル番号")
        else:
            ax.set_xlabel("サンプル番号"); ax.set_ylabel(y_col)
    ax.grid(True, linestyle="--", alpha=0.4)
    fig.tight_layout()
    return fig

def make_scatter_cumulative_fig(df: pd.DataFrame, x: str, flip: bool = False):
...
def make_scatter_counts_fig(df: pd.DataFrame, x: str, flip: bool = False):
    cnt = df[[x]].dropna().groupby(x, dropna=False).size().reset_index(name="データ数")
    fig, ax = plt.subplots(figsize=(6, 3))
    if flip: ax.scatter(cnt["データ数"], cnt[x], s=12)
    else:    ax.scatter(cnt[x], cnt["データ数"], s=12)
    if flip:
        ax.set_title(f"{x} vs 件数"); ax.set_xlabel("データ数"); ax.set_ylabel(x)
    else:
        ax.set_title(f"{x} ごとの件数"); ax.set_xlabel(x); ax.set_ylabel("データ数")
    fig.tight_layout(); return fig
...
def make_running_total_fig(df: pd.DataFrame, value_col: str, x_col: str | None = None,
    ...
    if x_col:
        ...
        X = df2[x_col]; x_label = x_col
    else:
        mask = df[value_col].notna()
        df2 = df.loc[mask, [value_col]].copy()
        df2["行番号"] = np.arange(1, len(df2) + 1)
        X = df2["行番号"]; x_label = "行番号"
    df2["小計"] = pd.to_numeric(df2[value_col], errors="coerce").fillna(0).cumsum()
    Y = df2["小計"]
    fig, ax = plt.subplots(figsize=(6, 3))
    if flip: ax.plot(Y, X, marker="o", linewidth=1)
    else:    ax.plot(X, Y, marker="o", linewidth=1)
    if fontname:
        if flip:
            ax.set_title(f"{value_col} の小計", fontname=fontname); ax.set_xlabel("小計", fontname=fontname); ax.set_ylabel(x_label, fontname=fontname)
        else:
            ax.set_title(f"{value_col} の小計", fontname=fontname); ax.set_xlabel(x_label, fontname=fontname); ax.set_ylabel("小計", fontname=fontname)
    else:
        if flip: ax.set_title(f"{value_col} の小計"); ax.set_xlabel("小計"); ax.set_ylabel(x_label)
        else:    ax.set_title(f"{value_col} の小計"); ax.set_xlabel(x_label); ax.set_ylabel("小計")
    fig.tight_layout(); return fig

# ========================= PDF生成 =========================
def _jp_paragraph_styles():
    styles = getSampleStyleSheet()
    title = ParagraphStyle('TitleJP', parent=styles['Heading1'],
                           fontName=RL_FONT_BOLD or styles['Heading1'].fontName,
                           fontSize=20, textColor=colors.HexColor('#1f77b4'),
                           alignment=1, spaceAfter=18)
    h2 = ParagraphStyle('H2JP', parent=styles['Heading2'],
                        fontName=RL_FONT_BOLD or styles['Heading2'].fontName,
                        fontSize=14)
    normal = ParagraphStyle('NormalJP', parent=styles['Normal'],
...
def _jp_table_style(header_bold=True):
    ...
    if RL_FONT_REG:  base.append(('FONTNAME', (0, 0), (-1, -1), RL_FONT_REG))
    if header_bold and RL_FONT_BOLD: base.append(('FONTNAME', (0, 0), (-1, 0), RL_FONT_BOLD))
    base += [('BACKGROUND', (0, 0), (-1, 0), colors.grey),
             ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke)]
    return TableStyle(base)

def generate_pdf(df_clean: pd.DataFrame, stats_df: pd.DataFrame | None,
                 sheet_name: str, masks: dict, pdf_opts: dict | None = None) -> bytes | None:
    """pdf_opts keys:
       include_hist / include_scatter_cum / include_scatter_cnt / scatter_x / scatter_flip
       include_running_total / rt_value_col / rt_x_col / rt_flip
       include_series_scatter / series_y_col / series_flip
    """
    ...
    include_series_scatter = pdf_opts.get('include_series_scatter', False)
    series_y_col = pdf_opts.get('series_y_col', None)
    series_flip = pdf_opts.get('series_flip', False)

    try:
        tmp_pdf = tempfile.NamedTemporaryFile(delete=False, suffix=".pdf")
        pdf_path = tmp_pdf.name; tmp_pdf.close()
        tmp_imgs = []
        ...
        t_over = Table(overview, colWidths=[3 * inch, 2 * inch]); t_over.setStyle(_jp_table_style())
        ...
        if stats_df is not None and not stats_df.empty:
            ...
            t_stats = Table(data, repeatRows=1); t_stats.setStyle(_jp_table_style())
            ...
            if ncols:
                ...
                for i, c in enumerate(ncols):
                    ...
                    if MPL_FONT_NAME:
                        ...
                        ax.set_xlabel(c, fontname=MPL_FONT_NAME); ax.set_ylabel("Frequency", fontname=MPL_FONT_NAME)
                    else:
                        ax.set_title(f"{c} Distribution"); ax.set_xlabel(c); ax.set_ylabel("Frequency")

                    tmp_img = tempfile.NamedTemporaryFile(delete=False, suffix=".png")
                    fig.savefig(tmp_img.name, bbox_inches="tight", dpi=110); plt.close(fig)
        ...
        # 時系列散布図（サンプル番号×値）
        if include_series_scatter and series_y_col:
            story += [PageBreak(), Paragraph("Time-series Scatter (サンプル番号×値)", h2_style), Spacer(1, 0.1 * inch)]
            fig = make_series_scatter_fig(df_clean, series_y_col, fontname=MPL_FONT_NAME, flip=series_flip)
            tmp_img = tempfile.NamedTemporaryFile(delete=False, suffix=".png")
            fig.savefig(tmp_img.name, bbox_inches="tight", dpi=110); plt.close(fig)
            tmp_imgs.append(tmp_img.name)
            story += [Image(tmp_img.name, width=5 * inch, height=2.5 * inch), Spacer(1, 0.2 * inch)]
        ...
        if (include_scatter_cum or include_scatter_cnt) and scatter_x:
            ...
            if include_scatter_cum:
                fig = make_scatter_cumulative_fig(df_clean, scatter_x, flip=scatter_flip)
                tmp_img = tempfile.NamedTemporaryFile(delete=False, suffix=".png")
                fig.savefig(tmp_img.name, bbox_inches="tight", dpi=110); plt.close(fig)
                tmp_imgs.append(tmp_img.name)
                story += [Image(tmp_img.name, width=5 * inch, height=2.5 * inch), Spacer(1, 0.2 * inch)]
            if include_scatter_cnt:
                fig = make_scatter_counts_fig(df_clean, scatter_x, flip=scatter_flip)
                tmp_img = tempfile.NamedTemporaryFile(delete=False, suffix=".png")
                fig.savefig(tmp_img.name, bbox_inches="tight", dpi=110); plt.close(fig)
                tmp_imgs.append(tmp_img.name)
                story += [Image(tmp_img.name, width=5 * inch, height=2.5 * inch), Spacer(1, 0.2 * inch)]
        ...
        if include_running_total and rt_value_col:
            story += [PageBreak(), Paragraph("Running Total (小計)", h2_style), Spacer(1, 0.1 * inch)]
            fig = make_running_total_fig(df_clean, value_col=rt_value_col, x_col=rt_x_col,
                                         fontname=MPL_FONT_NAME, flip=rt_flip)
            tmp_img = tempfile.NamedTemporaryFile(delete=False, suffix=".png")
            fig.savefig(tmp_img.name, bbox_inches="tight", dpi=110); plt.close(fig)
            tmp_imgs.append(tmp_img.name)
            story += [Image(tmp_img.name, width=5 * inch, height=2.5 * inch), Spacer(1, 0.2 * inch)]
...
def main():
    ...
    up = st.sidebar.file_uploader("Excelファイル (.xlsx/.xls)", type=["xlsx", "xls"])
    st.sidebar.markdown("---")
    st.sidebar.subheader("🧾 CSVの文字コード")
    st.sidebar.radio("Excelで文字化けする場合は『cp932』を選択してください。",
                     ["cp932 (Shift_JIS/Excel向け)", "utf-8-sig (UTF-8/BOM)"],
                     index=0, key="csv_enc_label")
    ...
    with tab2:
        ...
        if stats_df is not None:
            ...
            with c3: st.write(""); do = st.button("計算する", type="primary")
            if do:
                if usl <= lsl:
    ...
    with tab3:
        ...
        if not ncols:
            ...
            if base_val != "" and pd.notna(base_val):
                try: st.info(f"💡 基準値: {float(base_val):.3f}（USL/LSLへ転記可）")
                except: st.info(f"💡 基準値: {base_val}")
            c1, c2, c3 = st.columns([2, 2, 1])
            ...
            with c3:
                if base_val != "" and pd.notna(base_val):
                    ...
                    if st.button("📋 基準値を転記", key=f"copy_{col}"):
                        ...
                        except: st.error("基準値を数値に変換できません。")
            ...
            if not s.empty:
                ...
                if usl_v is not None and lsl_v is not None and usl_v > lsl_v:
                    ...
                    if sd > 0:
                        ...
                        st.markdown("#### 🎯 工程能力指数")
                        d1, d2 = st.columns(2); d1.metric("Cp", f"{cp:.3f}"); d2.metric("Cpk", f"{cpk:.3f}")
    ...
    with tab4:
        ...
        if len(ncols) < 1:
            ...
            # デフォルトを「時系列（サンプル番号×値）」に変更
            ctop1, ctop2 = st.columns([3, 1])
            with ctop1:
                mode = st.radio(
                    "表示モード",
                    ["時系列（サンプル番号×値）", "通常（列を選択）",
                     "累積データ数（1..N）", "各値の件数（Xごとの件数）", "小計（ランニングサム）"],
                    index=0, horizontal=True
                )
            with ctop2:
                flip_axes = st.checkbox("XとYを反転（X↔Y）", value=False)

            # ① 時系列（サンプル番号×値） ← デフォルト
            if mode == "時系列（サンプル番号×値）":
                y = st.selectbox("値（Y軸：数値列）", ncols, key="series_y_col")
                st.caption("上からの並び順＝時系列として、サンプル番号（1..N）×値の散布図を描画します。")
                if st.button("📈 作成", type="primary", key="scatter_btn_series"):
                    df2 = df_clean[[y]].dropna().copy()
                    df2["サンプル番号"] = np.arange(1, len(df2) + 1)
                    if flip_axes:
                        fig = px.scatter(df2, x=y, y="サンプル番号", title=y)
                        fig.update_layout(xaxis_title=y, yaxis_title="サンプル番号")
                    else:
                        fig = px.scatter(df2, x="サンプル番号", y=y, title=y)
                        fig.update_layout(xaxis_title="サンプル番号", yaxis_title=y)
                    fig.update_traces(mode="markers", marker_size=6)
            ...
            # ② 通常
                ...
                if st.button("📈 作成", type="primary", key="scatter_btn_norm"):
                    ...
                    fig = px.scatter(df_clean, x=plot_x, y=plot_y, title=f"{plot_y} vs {plot_x}")
            ...
            # ③ 累積データ数
                ...
                if st.button("📈 作成", type="primary", key="scatter_btn_cum"):
                    ...
                    df2["データ数"] = np.arange(1, len(df2) + 1)
                    if flip_axes:
                        fig = px.scatter(df2, x="データ数", y=x, title=f"{x} vs 累積データ数")
                        fig.update_layout(xaxis_title="データ数", yaxis_title=x)
                    else:
                        fig = px.scatter(df2, x=x, y="データ数", title=f"累積データ数 vs {x}")
                        fig.update_layout(xaxis_title=x, yaxis_title="データ数")
                    fig.update_traces(mode="markers"); st.plotly_chart(fig, use_container_width=True)
            ...
            # ⑤ 小計（ランニングサム）
                ...
                if st.button("📈 作成", type="primary", key="scatter_btn_running"):
                    if x_use:
                        ...
                        df2 = df_clean.loc[mask, [y_val]].copy(); df2["行番号"] = np.arange(1, len(df2) + 1)
                    df2["小計"] = pd.to_numeric(df2[y_val], errors="coerce").fillna(0).cumsum()
                    px_x, px_y = ("小計", x_use or "行番号") if flip_axes else (x_use or "行番号", "小計")
                    fig = px.line(df2, x=px_x, y=px_y, title=f"{y_val} の小計（ランニングサム）")
    ...
    with tab5:
        ...
        if not ncols:
            ...
            if st.button("📈 生成", type="primary"):
                ...
                if gtype == "散布図": fig = px.scatter(df_clean, x=px_x, y=px_y, title=f"{px_y} vs {px_x}")
                elif gtype == "折れ線": fig = px.line(df_clean, x=x, y=y, title=f"{y} の推移")
                elif gtype == "棒":     fig = px.bar(df_clean, x=x, y=y, title=f"{y} の棒グラフ")
                else:                   fig = px.area(df_clean, x=x, y=y, title=f"{y} の面グラフ")
    ...
    pdf_include_series = st.checkbox("散布図（サンプル番号×値）を含める", value=True)  # ← デフォルトON
    ncols = numeric_cols(df_clean)
    if ncols:
        series_y_for_pdf = st.selectbox("PDF用：時系列散布図の列（Y）", ncols, index=0)
        series_flip_pdf = st.checkbox("（PDF）時系列散布図のXとYを反転（X↔Y）", value=False)
    else:
        series_y_for_pdf, series_flip_pdf = None, False

    st.markdown("##### 追加図（必要に応じて）")
    ...
    pdf_scatter_flip = st.checkbox("（PDF）上記散布図のXとYを反転（X↔Y）", value=False)
    scatter_x_for_pdf = st.selectbox("PDF用：X軸列（累積/件数 図用）", df_clean.columns.tolist(), index=0)

    st.markdown("##### 小計（ランニングサム）をPDFに含める")
    pdf_include_rt = st.checkbox("小計（ランニングサム）チャートを含める", value=False)
    if ncols:
        rt_value_col_pdf = st.selectbox("小計対象の数値列（Y）", ncols, index=0)
    else:
        rt_value_col_pdf = None
    rt_x_col_pdf = st.selectbox("時系列X（任意/未選択=行番号）", ["（未選択）"] + df_clean.columns.tolist(), index=0)
    rt_x_col_pdf = None if rt_x_col_pdf == "（未選択）" else rt_x_col_pdf
            ...
            with st.spinner("PDFを生成中..."):
                ...
                pdf = generate_pdf(
                    ...
                    pdf_opts={
                        ...
                        'include_series_scatter': pdf_include_series,
                        'series_y_col': series_y_for_pdf,
                        'series_flip': series_flip_pdf,
